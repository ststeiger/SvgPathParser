...
namespace PdfSharp.Xps.Parsing
{
  partial class XpsParser
  {
    ...
    /// </summary>
    public PathGeometry ParsePathGeometry(string data)
    {
#if DEBUG_
      ...
      if (data.StartsWith("M 20,100 C 45,50 70,150 95,100 S 145,"))
        Debugger.Break();
#endif
            // Replace space with "," - replace "-" with ",-", replace ",," with "," and do a bit of trimming – 
            // https://stackoverflow.com/questions/206717/how-do-i-replace-multiple-spaces-with-a-single-space-in-c
            data = System.Text.RegularExpressions.Regex.Replace(data, @"\s+", " ", System.Text.RegularExpressions.RegexOptions.Multiline);
            data = data.Replace(" ", ",");
            data = data.Replace("-", ",-");
            data = data.Replace(",,", ",");
